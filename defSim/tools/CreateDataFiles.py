...
class DataFileCreator(ABC):
    ...
    Inherit from this class and implement the create_file method to generate the desired output file for the output 
    table.
    """

    @abstractmethod
    def create_file(self, output_table: pd.DataFrame, output_path: str or Path, **kwargs):
        """
...
def create_tickwise_files(tickwise_dataframes, output_folder: Path, realization: DataFileCreator, **kwargs):
    ...
    :param output_folder: Path to folder where output files are stored.
...
def create_data_files(output_table: pd.DataFrame,
                      output_folder_path: str or Path = None,
                      output_file_name: str = 'outputfile',
                      **kwargs):
    """
    ...
    :param output_table: A Pandas data frame.
    :param output_folder_path: Path to folder where output files are stored.
    ...
    realizations = [i.lower() for i in pathlib.Path(output_file_name).suffixes]
    if not realizations:  # when list is empty
        realizations.append('.csv')

    if ".pickle" in realizations:
        PickleFileCreator().create_file(output_table=output_table, output_path=output_path, **kwargs)
        create_tickwise_files(tickwise_dataframes=tickwise_dataframes, output_folder=output_folder,
                              realization=PickleFileCreator(), **kwargs)

    if ".csv" in realizations:
        CSVFileCreator().create_file(output_table=output_table, output_path=output_path, **kwargs)
        create_tickwise_files(tickwise_dataframes=tickwise_dataframes, output_folder=output_folder,
                              realization=CSVFileCreator(), **kwargs)
    ...
    if any([i in realizations for i in [".hdf", ".hdf5"]]):
        ...
        create_tickwise_files(tickwise_dataframes=tickwise_dataframes, output_folder=output_folder,
                              realization=HDF5FileCreator(), **kwargs)
    ...
    if any([i in realizations for i in [".stata", ".dta"]]):
        ...
        create_tickwise_files(tickwise_dataframes=tickwise_dataframes, output_folder=output_folder,
                              realization=StataFileCreator(), **kwargs)
    ...
    if any([i in realizations for i in [".markdown", ".md"]]):
        ...
        create_tickwise_files(tickwise_dataframes=tickwise_dataframes, output_folder=output_folder,
                              realization=MarkdownFileCreator(), **kwargs)
