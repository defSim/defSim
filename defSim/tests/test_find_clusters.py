...
class test_find_clusters(TestCase):
    ...
    assert ds.OutputMeasures.find_clusters(G) == [2, 1, 1]
    # the list of sizes of clusters in which influence is still possible:
    assert ds.OutputMeasures.find_clusters(G, strict_zones=True) == [4]
    # the list of sizes of clusters with reasonably similar attribute profiles
    assert ds.OutputMeasures.find_clusters(G, cluster_dissimilarity_threshold=.5) == [2, 2]